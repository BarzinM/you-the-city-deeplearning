...
def getLabels(mat_file):
    import h5py
    f= h5py.File(mat_file,'r')
    print(f["digitStruct"].items())
    ...
    for file_number in range(30):
        data_dict = {}
        name = f[names_dataset[file_number][0]]
        file_name = ''.join(chr(x) for x in name)
        print(file_name)
        labels = f[bbox_dataset[file_number][0]]
        # print(labels)
        properties = f[bbox_dataset[file_number][0]]["label"]
        labels = []
        for label in properties:
            ...
            try:
                ...
                raise
        print(labels)
        # print(properties[:].squessze())
        # print(f[properties])
        # values = [int(f[ref][:][0,0] for ref in labels)]
...
if __name__ == "__main__":
    ...
    train_labels = getLabels('train/digitStruct.mat')
