    ...
    def makeGraph(self):
        ...
        with self.graph.as_default():
            ...
            weight_3 = tf.Variable(tf.truncated_normal(
                [5 * new_image_size**2 * depth_1, self.num_labels], stddev=0.1))
            bias_3 = tf.Variable(tf.constant(.05, shape=[self.num_labels]))
            ...
            def splitColumns(tensor,sizes):
                ...
                sliced_tensors = []
                for size in sizes:
                    sliced_tensors.append(tf.slice(tensor,[0,start],[self.batch_size,size]))
                    start += size
                return sliced_tensors
            
            sizes = [6]+[10]*5
            seperated_logits = splitColumns(logits,sizes)
            seperated_labels = splitColumns(self.label_flow,sizes)
            print("Seperated shapes are",[label.get_shape().as_list() for label in seperated_labels])
            ...
            loss = multiLabelLoss(seperated_logits,seperated_labels)
    ...
    def train(self, num_steps, generator, validation_batches, test_batches):
        with tf.Session(graph=self.graph) as session:
            ...
            for step in range(num_steps):
                ...
                if step % self.report_step == 0:
                    ...
                    for batch in range(0, validation_batches):
                        ...
                        valid_accuracy += self.batchEvaluation(
            ...
            for batch in range(0, test_batches):
                valid_data, valid_labels = next(generator)
                test_accuracy += self.batchEvaluation(valid_data, valid_labels)
